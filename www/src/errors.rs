...
pub async fn make_render_error_middleware<NF: IntoResponse, IE: IntoResponse>(
    not_found: impl AsyncFn() -> NF,
    internal_error: impl AsyncFn() -> IE,
) -> impl AsyncFnMut(Request<Body>, Next) -> Response {
    async move |request: Request<Body>, next: Next| -> Response {
        ...
        if let Some(handler_error) = response.extensions_mut().remove::<HandlerError>() {
            ...
            match handler_error {
                ...
                HandlerError::InternalError => {
                    let mut response = internal_error().await.into_response();
